...
#returns the difference between the price at start and the average over the subsequent time period
def get_average_difference_exchange_rate(start_time):
	total = 0.0
	count = 0
	file_num = (start_time - first_trade_day) / seconds_in_day
	read_next_file = True
	...
	while (read_next_file):
		...
		try:
			...
			read_next_file = False
			return -1
	return (total / count) - start_price
...
with open(comments_filepath, 'rb') as comments_file:
	with open(output_filepath, 'a') as output_file:
		...
		for line in comments_reader:
			...
			average = get_average_difference_exchange_rate(time)
			i += 1
			if (average == -1):
				continue
